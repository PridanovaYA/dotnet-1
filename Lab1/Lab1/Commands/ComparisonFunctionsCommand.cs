...
namespace Lab1.Commands
{
    public class ComparisonFunctionsCommand : Command<ComparisonFunctionsCommand.ComparisonFunctionsSettings>
    {
        ...
        {
            int index1 = AnsiConsole.Prompt(new TextPrompt<int>("[red]Enter the index of the first function:[/]"));
            int index2 = AnsiConsole.Prompt(new TextPrompt<int>("[red]Enter the index of the second function:[/]"));

            try
            {
                if (_functionsRepository.ComparisonFunctions(index1, index2))
                {
                    AnsiConsole.MarkupLine("[red]Functions are equal.[/]");
                }
                else
                {
